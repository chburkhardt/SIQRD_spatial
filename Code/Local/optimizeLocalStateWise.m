...
function retval = optimizeLocalStateWise (input1, input2)
  ...
  parameter.optimizationMode = "Germany-and-Counties"; %"Counties"|"Germany"|"Germany-and-Counties"
  parameter.folderName = "2020-06-04_PSO_slopeFit";
  parameter.startDate = datenum([2020, 03, 02]);
  parameter.betaStateWise	= true;		
  parameter.wRatioGlobStates = 0.5;%.5; % 1 is fully statewise, 0 is only global
  parameter.wRatioID = 0.9; % 1 is only infected, 0 only death
  parameter.wISlope = 0.002; %0.002 % weight for slope of infected last date
...
function retval = evalSirLocal(Xpara, paraNames, parameter, rkiData, showPlots, writeTexTable)  
 ...
 base_Q = (sumIsimStartP2/sumIsimStartP1)^(1/shiftDays);
  ...
  % Spline for simulation results in same time format as RKI data
  % weights
  wI = 1;
  wD = 1;
  ...
  % relevant timespan for the fit is the overlapping time between RKI data and simulation results
