...
function generateVTK (cities, index, folder, parameter, distances, time)  
  ...
  SevenDayIncidenceRKI = zeros(length(cities),1);
  SevenDayIncidenceSim = zeros(length(cities),1);
  ...
  betaCitywise = sir_eqn_spatial ("betas", parameter, time,cities);
  betaccCitywise = sir_eqn_spatial ("getBetaCC", parameter, time,cities);
  ...
  for i=1:length(cities)
    ...
    SevenDayIncidenceRKI(i) = cities{i}.SevenDayIncidenceRKI;
    SevenDayIncidenceSim(i) = cities{i}.SevenDayIncidenceSim;
    SIR(i, :) = cities{i}.SIR;
  ...
  if strcmp(parameter.model, "SIREDmod")  
    vals=zeros(nPoints, size(SIR, 2)+6+2+2);  
  else
    vals=zeros(nPoints, size(SIR, 2)+5+2+2);  
  end
  
  iSPoints = 1;
  iSEl = 1;
  for i=1:length(cities)
    cityValues = [betaCitywise(i), betaccCitywise(i), darkFiguresCitywise(i),...
    ags(i), populationDensity(i),...
    population(i), infectedCumRKI(i), SIR(i,:), SevenDayIncidenceRKI(i), ...
    SevenDayIncidenceSim(i)];
    if strcmp(parameter.model, "SIREDmod")    
  ...
  switch parameter.model
    case "SIRH"
      names = {"Beta","BetaCC","Darkfigure", "AGS", "Populationdensity", "Population","InfectedRKI",...
      "Susceptible", "Exposed", "Removed", "Infectious","has_symptoms",...
      "in_hospital", "needs_intensive_care", "Death", "Discovered", "SevenDayIncidenceRKI", ...
      "SevenDayIncidenceSim"};
    case {"SIR", "SIRED", "SIREDYO","SIREDLiterature"}
      names = {"Beta","BetaCC","Darkfigure", "AGS", "Populationdensity","Population","InfectedRKI","S","I",...
      "R","E","D","S_risk","I_risk","R_risk","E_risk","D_risk"}; 
      names = names(1:size(vals, 2));   
    case "SIREDmod"
      names = {"Beta","BetaCC","Darkfigure", "AGS", "Populationdensity","Population","InfectedRKI",...
      "Susceptible", "Quarantined", "Recovered", "Infected", "Dead", "SevenDayIncidenceRKI", ...
      "SevenDayIncidenceSim", "Discovered"};
  endswitch
  ...
  if parameter.saveConnections
    ...
    ##    m=m(upperTriangle);
    ##    n=n(upperTriangle);
    ##    val=val(upperTriangle);
    ##    for i=1:length(val)
    ##      val(i) = val(i)^(-parameter.n_traffic).*...
    ##      biggestCity(m(i))^2 * biggestCity(n(i))^1./maxBiggestCity^3;
    ##    end
  ...
  ##  figure;
  ##  hold on;
  ##  for i=1:length(linesConnections)
  ##    plot(pointsConnections(1,linesConnections(:,i)), pointsConnections(2,linesConnections(:,i)));
  ##  end
  
  
  
  if or(any(strfind(parameter.spatial, "europeanCapitals")),...
    parameter.saveConnections)
