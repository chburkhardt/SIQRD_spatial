...
function out = sir_spatial_seeds_new (parameterArray)
  if nargin == 0
    close all;
    ...
    switch parameter.model
      case {"SIR", "SIRED","SIREDmod", "SIREDYO","SIREDLiterature"}
          ...
          if ~parameter.seed
            ...
            %create matrix that contains information about seeds
            seeds = [parameter.seedDate, parameter.seedAGS', parameter.seedSize'];
            %sort seed matrix by date
            [~,idx] = sort(seeds(:,1)); % sort just the first column
            seedsSorted = seeds(idx,:);
            %Sort out dates that occur multiple times
            [C,ia,ic] = unique(seedsSorted(:,1));
            
            % check if total runtime is longer than last seed
            if seedsSorted(end,1)>=(parameter.startDate+parameter.totalRuntime)
              error("Last seed later than total runtime");
            end
            
            % run until first seed
            runTime = seedsSorted(1,1) - parameter.startDate;
            t = [0,runTime];
            x = ode45(get_dXdT, t, X_0, opt);  
            currentEndDate = runTime;
            
            % initialize vector with all dates (sorted out dates that occur more than once)
            interuptsODE = [seedsSorted(ia,1)', parameter.startDate + parameter.totalRuntime];
            %write a matrix that contains seed AGS and seed size for dates in each row          
            for i=1:length(ic)
              if i == 1
                run=1;
              else
              ...
              seedAGS(ic(i),run)=seedsSorted(i,2);
              seedSize(ic(i),run)=seedsSorted(i,3);
            end
            for i=1:length(interuptsODE)-1
              runTime = interuptsODE(i+1) - interuptsODE(i);
              t = [currentEndDate,currentEndDate+runTime];
              
              %get last values of x as start values for next interval
              X0 = x.y(:, end);
              
              % generate vector with number of additional detected infections
              X0seed = getSeedX0new(X0, spatialDistinction, seedAGS(i,:), seedSize(i,:), parameter);
              
              % solve next interval and combine results
              xTmp = ode45(get_dXdT, t, X0seed, opt);  
              currentEndDate += runTime;
              x.x = [x.x, xTmp.x(2:end)];
              x.y = [x.y, xTmp.y(:,2:end)];          
            endfor
          end
        end
      case "SIRH"
        ...
        if ~parameter.seed
         ...
         [initialHistory, X_0] = getHistory(X_0, parameter, 40, spatialDistinction,...
          ...
          for i=1:length(interuptsODE)-1
            ...
            % generate vector with number of additional detected infections
            X0seed = getSeedX0(spatialDistinction, seedAGS(i,:), seedSize(i,:));
            % calculate history for X0seed
            [seedHistory, X0seed] = getHistory(X0seed, parameter, 40, spatialDistinction,...
            distances, population, biggestCityInCounty);
            
            % Interpolation of the old solution at the required points
            lags = sir_eqn_spatial("lags");
            solAtLags = spline(x.x, x.y, max(x.x) - linspace(lags(1), lags(end), 40));
            newHistory = solAtLags - (1 - seedHistory);
            
            % solve next interval and combine results
            xTmp = ode23d_fixed(get_dXdT, t, newHistory(:,end), lags, newHistory, opt);
            currentEndDate += runTime;
            %yHist = x.y(:,runTime-length(newHistory):)
            %x.y = [x.y(:,1:runTime-length(newHistory), x.y(:,(runTime-length(newHistory)):runTime)-newHistory];
            for j = 1:size(x.y, 1)
              for k = 1:length(seedAGS(i,:))
                if spatialDistinction{j}.ags==seedAGS(i,k)
                  %find index of the day that is length(lags) days before the end of x.x
                  ind = find(x.x >= (max(x.x) - length(lags)) & x.x < (max(x.x)-(length(lags)-1)));
                  ind = ind(1);
                  %build a spline, that contains values of the new history with the length of 28 days
                  splineNH = spline(1:size(newHistory,2), newHistory(j,:), ...
                  linspace(1, size(newHistory,2), (length(x.x)-ind)));
                  %add history to x.y
                  x.y(j,ind+1:end) = splineNH;  
                  saveJ = j;
                end
              end
            end    
            ...
            %Interpolate between values before and after seed date
            ##            xInterp = [x.x(sizeX-2), x.x(sizeX), x.x(sizeX+2)];
            ##            yInterp = [x.y(saveJ,sizeX-2), x.y(saveJ,sizeX), x.y(saveJ,sizeX+2)];
            ##            xq = x.x(sizeX-2:sizeX+2);
            ##            x.y(saveJ,sizeX-2:sizeX+2) = interp1(xInterp, yInterp, xq, 'pchip');
            ##            plot(xTmp.x, xTmp.y(2,:));
          endfor
        end
