          ...
          if numel(varargin)<4, error('Not enough input arguments'); end
            ...
            if numel(varargin)<4, error('Not enough input arguments'); end
              ...
              switch upper(varargin{1})
                ...
                % if secondary output eg. additional lines
                if any(strcmpi(upper(varargin), 'LINES'))
                  ...
                  if posLinesKeyword > 1
                    ...
                    fprintf(fid,'\nLINES %d %d\n',nbLine,3*nbLine);                  
                    fprintf(fid,'2 %d %d\n', varargin{posLinesKeyword + 1}' - 1);
                  end
                end
                
                
                % write datavectors to file
                if length(varargin) < 6          
                  fclose(fid);
                  return;
                end                
                if any(strcmpi(upper(varargin), 'NAMES'))                  
                  names = varargin{ find(strcmpi(upper(varargin), 'NAMES')) + 1};
                end                
                ...
                if size(vals, 2) != length(names)
                  error("Missmatch number fields and names");
                end                  
##                if mod(n_elements,3)==1
##                  vals(n_elements+1:n_elements+2, :) = [mean(vals, 1); mean(vals, 1)];
##                elseif mod(n_elements,3)==2
##                  vals(end+1, :) = mean(vals, 1);
##                end
                nbvals = size(vals, 1);
                fprintf(fid, ['POINT_DATA ' num2str(nbvals), '\n']);          
                for i=1:length(names)
                  fprintf(fid, ['SCALARS ' names{i} ' float\n']);
                  fprintf(fid, ['LOOKUP_TABLE default\n']);
